...
func TestEnsureGCPRootModuleCreatesPhaseAWithoutMigratingLegacy(t *testing.T) {
	root := filepath.Join(t.TempDir(), "generated", "gcp")
	legacy := filepath.Join(root, "compute", "main.tf")
	writeGCP(t, legacy, `resource "google_compute_instance" "legacy" { name = "legacy" }
`)
	...
	assertPhaseAFiles(t, root)
	after, _ := os.ReadFile(legacy)
	if !bytes.Equal(original, after) {
		t.Fatal("legacy resource was modified during phase A")
	}
...
func assertPhaseAFiles(t *testing.T, root string) {
	t.Helper()
...
func writeGCP(t *testing.T, path, content string) {
	t.Helper()
